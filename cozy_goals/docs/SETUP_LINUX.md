...
## Optional: website blocking support

For website blocking, install Polkit / pkexec if it is not already available:
...
Cozy Goals uses `pkexec` only when applying or clearing `/etc/hosts` rules. App process blocking does not require root; it uses `pgrep` and `pkill` while the app is running.
