...
The blocking feature has two mechanisms:

1. App process blocking: Flutter periodically calls Python `enforce_apps`, which uses `pgrep -f` and `pkill -f` for locked process patterns. This works while Cozy Goals is running.
2. Website blocking: Flutter calls Python `apply_site_blocks`, which uses `pkexec` to write a reversible Cozy Goals section in `/etc/hosts`.

This keeps blocking local-first and visible. No server, browser account or cloud dependency is involved.
