...
class AppRepository {
  ...
  Future<int> totalFocusSecondsForDay(String day) async {
    final db = await _db;
    final rows = await db.rawQuery(
      'SELECT COALESCE(SUM(focusSeconds), 0) as total FROM goals WHERE goalDate = ?',
      [day],
    );
    return Sqflite.firstIntValue(rows) ?? 0;
  }
  ...
  Future<void> addGoalFocusSeconds(String goalId, int seconds) async {
    if (seconds <= 0) return;
    final db = await _db;
    await db.rawUpdate(
      'UPDATE goals SET focusSeconds = focusSeconds + ? WHERE id = ?',
      [seconds, goalId],
    );
  }
  ...
  Future<bool> isTimeBonusAwarded(String day) async {
    final db = await _db;
    final rows = await db.query('daily_time_bonus', where: 'day = ?', whereArgs: [day], limit: 1);
    return rows.isNotEmpty;
  }

  Future<void> markTimeBonusAwarded(String day, int xpAwarded) async {
    final db = await _db;
    await db.insert(
      'daily_time_bonus',
      {
  ...
  Future<List<BlockedApp>> blockedApps() async {
    final db = await _db;
    final rows = await db.query('blocked_apps', orderBy: 'kind ASC, name COLLATE NOCASE ASC');
